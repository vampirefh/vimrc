...
autocmd FileType cpp let b:dispatch = 'clang++ % -o %<.out -std=c++1z && ./%<.out'
autocmd FileType cpp nnoremap <buffer> <F7> :w<cr>:Dispatch clang++ % -o %<.out -std=c++1z<cr>
...
autocmd FileType go nnoremap <buffer> <F7> :w<cr>:Dispatch go build -o ./%<.out %<cr>
